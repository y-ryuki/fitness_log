...
class UserLogic
{
   ...
   */
    ...
    {
        $insertUser=[];
        $insertUser['userid'] = $usersData['userid'];
   ...
   * @return bool $err
   */
    public function validateUsers($usersData)
    {
        $err = [];
        if(!$usersData['userid'] = filter_input(INPUT_POST,'userid')){
            $err[] = 'ユーザーIDを記入してください';
        }
   ...
   */
    ...
    public function checkDuplicate($userData)
    {
...
SQL;
        ...
        $stmt = $this->pdo->prepare($sql);
        $params = array(':userid' => $userData['userid']);
...
SQL;
    ...
    public function updateUser($users,$current_userid)
    {
        $sql = <<< SQL
            UPDATE 
                users
